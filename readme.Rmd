...
Function `updateRainDB` fetches rainfall data for a selected time period from an in-house and an external server and adds it to a local data base (plain text file). As a first step, the user must manually create an NA-filled data base covering the full time period as template. 

```{r, echo=FALSE}
head(rainData)
``` 

The function will read this template
```R
...
- `overwriteOldDB`: If `FALSE`, generates a new data base (plain text file)
- `summerTime` data.frame(year=c(2020, 2021), 
